...
class FluxLight(LightEntity):
    ...
    def RGBWW_brightness(self, RGBWW_Data, brightness_255=None):
        ...
        if len(RGBWW_Data) == 5:
            ...
            total_brightness_255 = (
                color_brightness_255 + ww_brightness_255 + cw_brightness_255
            )
            max_brightness_255 = 765
            current_brightness_255 = round(total_brightness_255 / 3)
        else:
            total_brightness_255 = color_brightness_255 + ww_brightness_255
            current_brightness_255 = round(total_brightness_255 / 2)
            max_brightness_255 = 510

        if brightness_255:
            color_brightness_pct = color_brightness_255 / total_brightness_255
            ww_brightness_pct = ww_brightness_255 / total_brightness_255
            brightness_pct = brightness_255 / 255
            new_total_brightness_255 = brightness_pct * max_brightness_255

            if cw_brightness_255:
                cw_brightness_pct = cw_brightness_255 / total_brightness_255
                RGBWW[4] = round(new_total_brightness_255 * cw_brightness_pct)

            RGBWW[3] = round(new_total_brightness_255 * ww_brightness_pct)
            color_brightness_255 = round(
                new_total_brightness_255 * color_brightness_pct
            )
            hsv_data[2] = (color_brightness_255 / 255) * 100
            RGBWW[0:3] = list(color_util.color_hsv_to_RGB(*hsv_data))
            new_brightness = brightness_255

        else:
            new_brightness = current_brightness_255
            RGBWW = RGBWW_Data

        return (RGBWW, new_brightness)
